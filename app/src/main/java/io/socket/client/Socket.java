...
public class Socket extends Emitter {
    ...
    private static org.slf4j.Logger log = LoggerFactory.getLogger(Socket.class);
    ...
    private void onclose(String reason) {
        log.debug(String.format("close (%s)", reason));
    ...
    private void onevent(Packet<JSONArray> packet) {
        ...
        log.debug(String.format("emitting event %s", args));
            ...
            public void call(final Object... args) {
                EventThread.exec(new Runnable() {
                    ...
                    public void run() {
                        if (sent[0]) return;
                        sent[0] = true;
                        log.debug(String.format("sending ack %s", args.length != 0 ? args : null));
    ...
    private void onack(Packet<JSONArray> packet) {
        ...
        if (fn != null) {
            log.debug(String.format("calling ack %s with %s", packet.id, packet.data));
            fn.call(toArray(packet.data));
        } else {
            log.debug(String.format("bad ack %s", packet.id));
        }
    }
    ...
    private void ondisconnect() {
        log.debug(String.format("server disconnect (%s)", this.nsp));
        this.destroy();
    ...
    public Socket close() {
        EventThread.exec(new Runnable() {
            ...
            public void run() {
                if (Socket.this.connected) {
                    log.debug(String.format("performing disconnect (%s)", Socket.this.nsp));
    ...
    private static Object[] toArray(JSONArray array) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            try {
                ...
                log.warn("An error occured while retrieving data from JSONArray", e);
                v = null;
            }
